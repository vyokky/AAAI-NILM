...
def load_dataset():
    ...
    tra_set_x = np.load(tra_x + '.npy')[:4000]
    tra_set_y = np.load(tra_y + '.npy')[:4000]
    val_set_x = np.load(val_x + '.npy')[:4000]
    val_set_y = np.load(val_y + '.npy')[:4000]
...
cost = tl.cost.binary_cross_entropy(y, y_)
